        ...
        <header class="page-header">
            <div class="text-container">
                ...
                <?php //unseen tickets Notification
                    ...
                    $sql_unseen = "SELECT Count(*) as unseen_count  FROM ticket join residence on ticket.resName = residence.resName where ticket_status = 'Resolved' and h_seen = 0 and hall_name = '$hallname';";
            ...
            <nav class="houses">
                <?php
                    ...
                    while ($residence = $residences_result->fetch_assoc()) {
                        ...
                        echo "<a href='hall_secretary_resolved.php?house_name={$residence['residences']}' class='house-link {$isActive}'>{$residence['residences']}</a>";
                    }
                ?>
            </nav>
                    ...
                    <tbody>
                        ...
                        <?php
                            if(isset($_REQUEST['house_name'])){
                                ...
                                $ticket_sql = "SELECT * FROM ticket WHERE ticket_status = 'Resolved' AND resName = '$housename' ORDER BY ticketID DESC;";
                                $ticket_result = $connection->query($ticket_sql);
                            }
                            else{
                                $ticket_sql = "SELECT * FROM ticket WHERE ticket_status = 'Resolved' AND resName = '$defaulthouse' ORDER BY ticketID DESC;";
                                $ticket_result = $connection->query($ticket_sql);
                            }
                            ...
                            if ($ticket_result->num_rows > 0) {
                                while ($row = $ticket_result->fetch_assoc())
                                {
                                    ...
                                    if (strtolower($row['ticket_status']) == "Resolved") {
                                    ...
                                    // echo "<td><a href='hall_secretary_resolved.php?ticket_ID={$row['ticketID']}&house_name={$activeHouse}'> Comment </a>";
                                    echo "<td><p><a href='hall_secretary_resolved.php?ticket_ID={$row['ticketID']}&house_name={$activeHouse}&mark_seen=1' id='viewButton'> View Details</a></p></td>";
                                    echo "<td><a href='updateStatus.php?ticket_ID={$row['ticketID']}&house_name={$activeHouse}&mark_seen=1' class='custom-button' onclick= \"return confirm('Are you sure you want to update ticket status')\">Close Ticket</a></td></tr>";
                                    echo "<tr>";
