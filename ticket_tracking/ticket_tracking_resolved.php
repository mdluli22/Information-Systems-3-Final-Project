...
// Notifications Code: Get count of unseen 'Resolved' tickets
$sql_unseen_resolved = "SELECT COUNT(*) AS unseen_count FROM systemsurgeons.ticket WHERE userName = '$userID' AND ticket_status = 'Resolved' AND s_seen = 0";
$result_unseen_resolved = $connection->query($sql_unseen_resolved);
$unseen_count_resolved = 0; // Default count
if ($result_unseen_resolved && $row = $result_unseen_resolved->fetch_assoc()) {
    $unseen_count_resolved = $row['unseen_count'];
}
// Store the unseen count in the session for use in the sidebar
$_SESSION['unseen_count_resolved'] = $unseen_count_resolved;
    ...
    <div class="container">
            ...
            <header>
                <div>
                    ...
                    <?php //unseen tickets Notification
                        $sql_unseen = "SELECT COUNT(*) AS unseen_count FROM systemsurgeons.ticket WHERE userName = '$userID' AND s_seen = 0 and (ticket_status = 'Resolved')";
                        ...
                        if ( $unseen_count > 0) {
                            echo "<p class='fade-out'>View and make comments on resolved tickets.<span style='color: #ef3e3e;'> You have $unseen_count new tickets</span></p>";
                        } else {
                            echo "<p class='fade-out'>View and make comments on resolved tickets.</p>";
                        }
                    ?>
                </div>
                <!-- Fix the logo size -->
            ...
            <div class="content-wrapper">
                ...
                <section class="ticket-list">
                    ...
                    <?php
                        ...
                        $sql = "SELECT ticketID, resName, ticket_status, s_seen FROM systemsurgeons.ticket where userName = '$userID' and (ticket_status = 'Resolved')";
                        $result = $connection -> query($sql); //execute query
                        ...
                        if ($result -> num_rows > 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                ...
                                if ($row['s_seen'] == 0) {
                                    echo "<td><a href='ticket_tracking_resolved.php?ticketID={$row['ticketID']}&mark_seen=1' class='details-button-unseen'>View Details</a></td>";
                                } else {
                                    echo "<td><a href='ticket_tracking_resolved.php?ticketID={$row['ticketID']}&mark_seen=1' class='details-button'>View Details</a></td>";
                                }
                    ...
                    $sql = "SELECT ticketID, resName, ticket_status FROM systemsurgeons.ticket where resName = '$residence' and (ticket_status = 'Resolved')";
                    $result = $connection -> query($sql); //execute query
                    ...
                    echo "<section class='scrollbar'>";
                        if ($result -> num_rows > 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                ...
                                $statusClass = "status " . strtolower($row['ticket_status']);
                                echo "<td><span class='{$statusClass}'><span class='circle'></span>&nbsp;&nbsp;{$row['ticket_status']}</span></td>";
                                echo "<td><a href='ticket_tracking_resolved.php?ticketID={$row['ticketID']}&mark_seen=1' class='details-button'>View Details</button></a></td>";
                                echo "</tr>";
                            } //end table
                            echo "</table>";
                        }
                                ...
                                if ($photos_result->num_rows > 0) {
                                    ...
                                    while ($photo = $photos_result->fetch_assoc()) {
                                        ...
                                        if ($ticketowner == $userID) {
                                            ...
                                            echo "<input type='hidden' name='page' value='resolved'>";
        ...
        <script> //Ticket notification code
            document.addEventListener('DOMContentLoaded', function() {
                const unseenCount = <?php echo $unseen_count_resolved; ?>;
