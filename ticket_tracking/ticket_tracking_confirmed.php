...
// Unseen tickets count for 'Confirmed' // Notifications Code
$sql_unseen_confirmed = "SELECT COUNT(*) AS unseen_count FROM systemsurgeons.ticket WHERE userName = '$userID' AND ticket_status = 'Confirmed' AND s_seen = 0";
$result_unseen_confirmed = $connection->query($sql_unseen_confirmed);
$unseen_count_confirmed = 0; // Default count
if ($result_unseen_confirmed && $row = $result_unseen_confirmed->fetch_assoc()) {
    $unseen_count_confirmed = $row['unseen_count'];
}
// Store the unseen count in the session for use in the sidebar
$_SESSION['uunseen_count_confirmed'] = $unseen_count_confirmed;
...
<body>
    ...
    <div class="container">
            ...
            <header>
                <div>
                    ...
                    <?php //unseen tickets Notification
                        $sql_unseen = "SELECT COUNT(*) AS unseen_count FROM systemsurgeons.ticket WHERE userName = '$userID' AND s_seen = 0 and (ticket_status = 'Confirmed')";
                        ...
                        if ( $unseen_count > 0) {
                            echo "<p class='fade-out'>View and make comments on confirmed tickets. <span style='color: #ef3e3e;'> You have $unseen_count new tickets</span></p>";
                        } else {
                            echo "<p class='fade-out'>View and make comments on confirmed tickets.</p>";
                        }
                    ?>
                </div>
                <!-- Fix the logo size -->
            ...
            <div class="content-wrapper">
                ...
                <section class="ticket-list">
                    ...
                    <?php
                        ...
                        $sql = "SELECT ticketID, resName, s_seen, ticket_status FROM systemsurgeons.ticket where userName = '$userID' and (ticket_status = 'Confirmed')";
                        $result = $connection -> query($sql); //execute query
                        ...
                        if ($result -> num_rows > 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                ...
                                if ($row['s_seen'] == 0) {
                                    echo "<td><a href='ticket_tracking_confirmed.php?ticketID={$row['ticketID']}&mark_seen=1' class='details-button-unseen'>View Details</a></td>"; // If ticket is unseen
                                } else {
                                    echo "<td><a href='ticket_tracking_confirmed.php?ticketID={$row['ticketID']}&mark_seen=1' class='details-button'>View Details</a></td>"; // If the ticket has been seen
                                }
                                echo "</tr>";
                            } //end table
                            echo "</table>";
                    ...
                    $sql = "SELECT ticketID, resName, ticket_status FROM systemsurgeons.ticket where resName = '$residence' and (ticket_status = 'Confirmed')";
                    $result = $connection -> query($sql); //execute query
                    ...
                    echo "<section class='scrollbar'>";
                        if ($result -> num_rows > 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                ...
                                $statusClass = "status " . strtolower($row['ticket_status']);
                                echo "<td><span class='{$statusClass}'><span class='circle'></span>&nbsp;&nbsp;{$row['ticket_status']}</span></td>";
                                echo "<td><a href='ticket_tracking_confirmed.php?ticketID={$row['ticketID']}&mark_seen=1' class='details-button'>View Details</a></td>";
                                echo "</tr>";
                            } //end table
                            echo "</table>";
                        }
                            ...
                            if (isset($_GET['ticketID'])) {
                                ...
                                if ($photos_result->num_rows > 0) {
                                    ...
                                    while ($photo = $photos_result->fetch_assoc()) {
                                        ...
                                        if ($ticketowner == $userID) {
                                            ...
                                            echo "<input type='hidden' name='page' value='confirmed'>";
                                    ...
                                    while ($comment = $comments_result->fetch_assoc()) {
                                        ...
                                        if ($comment['userName'] == $userID) {
                                            echo "<form action='soft_delete_comment.php' method='POST' style='display:inline;'>
                                                    ...
                                                    <input type='hidden' name='page' value='confirmed'>"; // tells the form handler which page to return to
                                            echo   "&nbsp;&nbsp;&nbsp;&nbsp;<button type='submit' class='delete-button'><i class='fa fa-trash' aria-hidden='true'></i></button>
                                                </form>";
                                        }
                                        echo "</div>";
                                ...
                                if ($userID == $ticketowner){
                                    echo "<form action='submit_comment.php' method='POST'>
                                        ...
                                        <input type='hidden' name='page' value='confirmed'>
        ...
        <script> // Ticket notification code
            document.addEventListener('DOMContentLoaded', function() {
                const unseenCount = <?php echo $unseen_count_confirmed; ?>;
